...
(function () {
  ...
  async function viewCandidate(id) {
    ...
    const refsLink = location.origin + '/refs/' + c.token;
    const refAskPanel = '<div class="panel" style="margin:.2rem 0 1rem"><strong>📨 Ask ' + esc(fn) + ' for their references</strong><div class="muted" style="font-size:.9rem;margin:.2rem 0 .55rem">After the interview, send a link where they pop in their referees — the names load straight back here, then you send each referee the form below.</div><div class="link-box"><code>' + esc(refsLink) + '</code><button class="btn btn-primary btn-sm" id="copyRefsLink">Copy</button></div>' + ((c.email || c.phone) ? '<div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.5rem">' + (c.email ? '<button class="btn btn-ghost btn-sm" id="askRefsEmail">✉️ Email it</button>' : '') + (c.phone ? '<button class="btn btn-ghost btn-sm" id="askRefsSms">📱 Text it</button>' : '') + '</div>' : '<div class="muted" style="font-size:.84rem;margin-top:.4rem">Add an email or mobile for ' + esc(fn) + ' (up top) to send it in one tap.</div>') + '</div>';
    const refSection = '<div class="section-title" style="margin-top:1.8rem"><h3>📞 Reference checks</h3><button class="btn btn-primary btn-sm" id="addReferee">+ Add a referee</button></div>' + refAskPanel +
    ...
    const crl = $('#copyRefsLink'); if (crl) crl.onclick = () => { if (navigator.clipboard) navigator.clipboard.writeText(refsLink); toast('Reference link copied — send it to ' + fn); };
    const are = $('#askRefsEmail'); if (are) are.onclick = async () => reportSend(await api('POST', '/candidates/' + id + '/request-references', { channel: 'email' }), 'email');
    const ars = $('#askRefsSms'); if (ars) ars.onclick = async () => reportSend(await api('POST', '/candidates/' + id + '/request-references', { channel: 'sms' }), 'sms');
